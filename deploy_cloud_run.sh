...
update_yaml() {
    ...
    sed "s|GCP_REGION_PLACEHOLDER|${REGION}|g" | \
    sed "s|CASE_IDS_PLACEHOLDER|${CASE_IDS}|g" | \
    sed "s|RECIPIENT_EMAIL_PLACEHOLDER|${RECIPIENT_EMAIL}|g" | \
    sed "s|POLL_INTERVAL_PLACEHOLDER|${POLL_INTERVAL:-5m}|g" | \
    sed "s|STATE_FILE_DIR_PLACEHOLDER|${STATE_FILE_DIR:-/tmp/case-tracker-states/}|g" | \
    sed "s|AUTO_LOGIN_PLACEHOLDER|${AUTO_LOGIN:-true}|g" | \
